function [st,Lt,C1_t,C2_t,Pt,Qt] = main_fun(C_inil,LG_inil,C1_obj,C2_obj,C1_inil,C2_inil,lambda1,lambda2,gamma,mu,SolTh,epsilon,dim,N)
...
for i = 1:1:Maxite
    ...
    if i == 1
        ...
        grad = CalculateGrad_pro(C1_obj,C2_obj,Lt,Lt_next,epsilon,lambda1,lambda2,mu,N,i);
        Lt = Lt_next;
    ...
    [lambda1,lambda2] = lambda_update(C1_t,C1_obj,C2_t,C2_obj,epsilon,Lt,Lt_next,dim,N);
...
%%update lambda1 and lambda2
function [lambda1,lambda2] = lambda_update(C1_t,C1_obj,C2_t,C2_obj,epsilon,Lt,Lt_next,dim,N)
    sum_g = 0;
    Pt = PressSol(Lt,dim,N);
    Qt = graphLQ(Pt,Lt);
    [C1,C2,LC] = graphLC_pro(Pt,Lt,Qt,N);
    loss = ((C1_t-C1_obj-epsilon)^2-(C1-C1_obj-epsilon)^2)+((C2_t-C2_obj-epsilon)^2-(C2-C2_obj-epsilon)^2);
    for i = 1:1:dim-1
        if Lt(i,i+1)~=0
            sum_g = sum_g+loss/(Lt_next(i,i+1)-Lt(i,i+1));
        end
    end
    for i = 1:1:dim-N
        if Lt(i,i+1)~=0
            sum_g = sum_g+loss/(Lt_next(i,i+N)-Lt(i,i+N));
        end
    end
    lambda1 = -(12.5+(C1_t-C1_obj-epsilon)^2+(C2_t-C2_obj-epsilon)^2)/sum_g;
    lambda2 = lambda1; 
end
